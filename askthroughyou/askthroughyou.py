...
class DoHHandler(BaseHTTPRequestHandler):
    ...
    def _status(self) -> str:
        ...
        identity_str = f"{identity['name']} {identity['surname']}" if identity else "not registered"
        return (
...
def _fetch_phonebook_url_from_github() -> str:
    ...
    try:
        ...
        log.warning("Could not fetch phonebook.json from GitHub: %s", e)
    return ""
...
def _ask_phonebook_url() -> str:
    ...
    # 2. GitHub
    print("Looking up phonebook URL from GitHub...")
    url = _fetch_phonebook_url_from_github()
    if url:
        save_config({"ATY_PHONEBOOK_URL": url})
        os.environ["ATY_PHONEBOOK_URL"] = url
        print(f"Phonebook URL found: {url}")
        return url

    # 3. Input interattivo
    print("\nCould not find phonebook URL automatically.")
    url = input("Enter phonebook URL: ").strip()
    if not url:
        return ""
    ...
    save_config({"ATY_PHONEBOOK_URL": url})
    os.environ["ATY_PHONEBOOK_URL"] = url
    print(f"URL saved to {_app_data_dir_early() / 'config.json'}")
    return url
...
def cmd_register(name: str, surname: str, country: str) -> int:
    ...
    if not phonebook:
        print("Error: phonebook URL not provided.")
        return 1
    ...
    try:
        ...
        print(f"Registered as {name} {surname} ({country.upper()})")
        print(f"Credentials saved to {IDENTITY_FILE}")
        return 0

    except Exception as e:
        print(f"Error connecting to phonebook: {e}")
        return 1
...
def cmd_search(name: str, surname: str, city: str = "") -> int:
    ...
    try:
        ...
        print(f"  status     : waiting for authorization")
        print(f"  expires at : {expires_str}")
        print(f"\nUse --status {rid} to check the response.")
        return 0
    except Exception as e:
        print(f"Error: {e}")
        return 1
...
def cmd_pending() -> int:
    """Mostra le richieste di contatto in arrivo e chiede accept/reject."""
    try:
        ...
        if not pending:
            ...
            return 0
        print(f"\n{len(pending)} incoming request(s):\n")
        for r in pending:
            created = time.strftime("%d/%m %H:%M", time.localtime(r["created_at"]))
            print(f"From    : {r['from_id']}")
            print(f"Search  : {r['search_query']}")
            print(f"Received: {created}")
            while True:
                risposta = input("Accept or reject? [accept/reject/block]: ").strip().lower()
                if risposta in ("accept", "reject", "block"):
                    break
                print("Type: accept, reject or block")
            result2 = cmd_authorize(r["request_id"], risposta)
            print()
        return 0
    except Exception as e:
        print(f"Error: {e}")
...
def cmd_authorize(request_id: str, action: str) -> int:
    ...
    if not mapped:
        ...
        print("Valid values: accept, accept_always, reject, block")
        return 1
    try:
        ...
        if not result.get("ok"):
            ...
            return 1
        labels = {
            "accept_once": "Accepted (once)",
            "accept_always": "Accepted (always — trusted contact)",
            "reject": "Rejected",
            "block": "Rejected and user blocked",
        }
            ...
            if token:
                ...
                try:
                    ...
                    if conn_result.get("ok") and conn_result.get("online"):
                        ...
                        print(f"Port      : {conn_result['port']}")
                        print(f"Country   : {conn_result.get('country_code', '?')}")
                    else:
                        print("Contact offline or node not registered.")
                except Exception as e:
                    print(f"Error fetching IP: {e}")
            else:
                print("Accepted but no token available.")
        elif s == "no_response":
            print("No response or request rejected.")
        return 0
    except Exception as e:
        print(f"Error: {e}")
        return 1
...
def cmd_run(country: str) -> int:
    ...
    if identity and pb_url and my_ip:
        try:
            ...
            log.info("Phonebook node_id updated")
        except Exception as e:
...
def main() -> int:
    ...
    if args.search:
        ...
        if not name or not surname:
            print("Usage: --search firstname lastname [city]")
            return 1
