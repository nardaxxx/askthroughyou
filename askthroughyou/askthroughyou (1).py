...
def _ask_phonebook_url() -> str:
    """Chiede interattivamente l'URL del phonebook e lo salva in config.json."""
    print("\nPhonebook URL non configurato.")
    print("Esempio: http://89.251.254.185:8091")
    url = input("Inserisci l'URL del phonebook ATY: ").strip()
    if not url:
        return ""
    # Normalizza
...
def cmd_status(request_id: str) -> int:
    ...
    try:
        ...
        if s == "accepted":
            ...
            exp = result.get("expires_at", 0)
            exp_str = time.strftime("%H:%M:%S", time.localtime(exp)) if exp else "?"
            if token:
                print(f"Token     : {token}  (valido fino alle {exp_str})")
                print(f"\nUsa --connect {token} per ottenere l'IP.")
        elif s == "no_response":
