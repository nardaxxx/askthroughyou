...
### Configure DNS in Brave (or any browser)

Once running, set this as your DNS-over-HTTPS provider:

```
http://127.0.0.1:53535/dns-query
```

In Brave: Settings → Privacy and Security → Security → Use secure DNS → Custom → paste the URL above.
...
For your node to be reachable from the internet, you must open **TCP port 35353** on your router.

**Single router:**
Add a port forward: external port 35353 → your server's local IP → port 35353.

**Double NAT (two routers in cascade):**
You need to open the port on both routers in sequence:
- Router 1 (main, e.g. Fritz!Box): port forward 35353 → Router 2's IP
- Router 2 (e.g. OpenWrt): port forward 35353 → server's local IP

**Fritz!Box specific:**
If you have a Fritz!Box as the main router, go to Internet → Port Sharing → select your downstream router → set it as Exposed Host (passes all traffic to the next router, which handles its own firewall).

**Without port forwarding:**
The node works as a client only — it can use the network but cannot be reached by others.
...
- Removes them automatically on disconnect
