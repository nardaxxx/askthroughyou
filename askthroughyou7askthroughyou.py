...
def cmd_register(name: str, surname: str, country: str) -> int:
    ...
    if not phonebook:
        print("Errore: ATY_PHONEBOOK_URL non configurato nel .env")
        print(f"Aggiungi: ATY_PHONEBOOK_URL=http://89.251.254.185:8091")
        return 1

    # Carica node_id se disponibile
    node_id = NODE_ID
...
def cmd_run(country: str) -> int:
    ...
    # Aggiorna node_id nel phonebook se registrato
    identity = load_identity()
    if identity and PHONEBOOK_URL and my_ip:
        try:
            payload = json.dumps({"node_id": NODE_ID}).encode("utf-8")
            req = urllib.request.Request(
                f"{PHONEBOOK_URL.rstrip('/')}/update",
                data=payload,
                headers={
                    "Content-Type": "application/json",
