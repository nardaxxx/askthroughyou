#!/usr/bin/env python3
"""
Ask Through You v1.3
--------------------
Distributed human-node DNS network.

v1.3 — multi-port fallback + HF SIGNAL 01
- Prova porte 35353 -> 8443 -> 8080 in ordine
- HF_SIGNAL: segnale emergenza minimo (50 byte)
- Funziona su router consumer standard
- Ottimizzato per rete degradata e batteria scarica
...
# Porte fallback per HF e connessione registry
# Su router consumer 8080/8443 passano come traffico web
HF_PORTS = [
    int(os.getenv("ATY_LISTEN_PORT", "35353")),
    int(os.getenv("ATY_LISTEN_PORT_2", "8443")),
    int(os.getenv("ATY_LISTEN_PORT_3", "8080")),
]

# Timeout aggressivi per rete degradata
CONNECT_TIMEOUT = int(os.getenv("ATY_CONNECT_TIMEOUT", "3"))
HF_TIMEOUT = int(os.getenv("ATY_HF_TIMEOUT", "5"))
...
_my_ip = ""
_my_country = ""
_registry_peer: Optional["Peer"] = None
...
def cleanup_dns_cache() -> None:
    now = time.time()
    with state_lock:
        expired = [k for k, v in dns_cache.items()
...
def send_hf_signal(
    ...
    """
    Invia segnale emergenza Human Flag attraverso la rete ATY.

    Prova le porte in ordine: 35353 -> 8443 -> 8080
    Payload minimo — funziona con rete degradata e batteria scarica.
    Timeout aggressivo: 5 secondi totali per porta.

    Args:
        signal_type: tipo segnale ('surrender', 'distress', 'sos')
        gps: coordinate GPS opzionali ('lat,lon')
        message: messaggio breve opzionale (max 256 char)

    Returns:
        True se il segnale è stato confermato dal server
    """
    with state_lock:
        candidates = list(known_peers)

    if not candidates:
        log.error("HF: no peers available")
        return False

    # Payload minimo — circa 80-100 byte
    payload = {
        "type": "HF_SIGNAL",
        "version": "HF01",
        "signal": signal_type[:32],
    ...
    log.info("HF: sending [%s] payload=%d bytes", signal_type, payload_size)

    # Prova ogni peer su ogni porta
    for peer in candidates:
        for port in HF_PORTS:
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(HF_TIMEOUT)
                sock.connect((peer.ip, port))
                send_line(sock, payload)
                resp = recv_line(sock, timeout=HF_TIMEOUT)
                sock.close()

                if resp and resp.get("type") == "HF_ACK" and resp.get("ok"):
                    log.warning(
                        "HF: signal [%s] confirmed via %s:%d",
                        signal_type, peer.ip, port,
                    )
                    return True
                else:
                    log.warning(
                        "HF: no ack from %s:%d", peer.ip, port
                    )

            except Exception as e:
                log.warning(
                    "HF: %s:%d failed (%s)", peer.ip, port, e
                )
                continue

    log.error("HF: all peers and ports failed — signal NOT delivered")
    return False


def cmd_hf(signal_type: str, gps: str, message: str) -> int:
    """
    Comando standalone: invia segnale HF senza avviare il nodo completo.
    Utile per uso in emergenza — minimo overhead, massima velocità.
    """
    global _my_ip, _my_country

    log.info("HF emergency mode — loading peers...")

    # Carica peers da cache locale prima (più veloce)
    peers = load_peer_cache()
    if not peers:
        log.info("No cache — fetching bootstrap...")
        peers = fetch_all_peers()

    if not peers:
        log.error("No peers available — cannot send HF signal")
        return 1

    with state_lock:
        known_peers.extend(peers)

    ok = send_hf_signal(signal_type, gps, message)
    if ok:
        print(f"✓ HF signal [{signal_type}] delivered")
        return 0
    else:
        print(f"✗ HF signal [{signal_type}] FAILED — no peer responded")
        return 1
...
def registry_connection_loop(geo: dict[str, str]) -> None:
    ...
    while running:
        ...
        # Prova ogni porta disponibile
        for port in HF_PORTS:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(CONNECT_TIMEOUT)
            try:
                sock.connect((registry.ip, port))
                register_msg = {
                    "type": "REGISTER",
                    "ip": _my_ip,
                    "port": NODE_PORT,
                ...
                log.info(
                    "Connected to registry %s (%s) via port %d",
                    registry.ip, registry.country_code, port,
                )
                connected = True
                ...
                while running:
                    ...
                    if msg is None:
                        try:
                            ...
                            log.warning("Registry connection lost")
                            break
                        continue
                    mtype = msg.get("type", "")
                    if mtype == "PEER_LIST":
                ...
                break  # connesso — esci dal loop porte

            except Exception as e:
                log.warning(
                    "Cannot connect to registry %s:%d: %s",
                    registry.ip, port, e,
                )
            finally:
                try:
        ...
        if running:
            if not connected:
                log.warning(
                    "Registry %s unreachable on all ports — trying next peer",
                    registry.ip,
                )
...
def resolve_via_peer(
    ...
    for port in HF_PORTS:
        ...
        try:
            ...
            log.warning("Peer %s:%d error: %s", peer.ip, port, e)
            continue
        finally:
            try:
                sock.close()
...
class DoHHandler(BaseHTTPRequestHandler):
    ...
    def _status(self) -> str:
        ...
        if peers == 0:
            ...
            peer_warning = (
                f"\n⚠️  No peers in {country} — "
                f"falling back to other countries"
            )

        registry_status = (
            f"{registry.ip} ({registry.country_code})"
            if registry else "DISCONNECTED"
        )
...
def cmd_run(country: str) -> int:
    ...
    threads = [
        ...
        threading.Thread(
            target=registry_connection_loop,
            args=(geo,),
            daemon=True,
        ),
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Ask Through You — distributed human-node DNS + HF emergency signals"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--country", "-c",
        metavar="XX",
        help="Target country (e.g.: CH, IT, DE)",
    )
    group.add_argument(
        "--list", "-l",
        action="store_true",
        help="Show available countries",
    )
    group.add_argument(
        "--hf",
        metavar="SIGNAL",
        help="Send HF emergency signal (surrender/distress/sos)",
    )
    parser.add_argument(
        "--gps",
        metavar="LAT,LON",
        default="",
        help="GPS coordinates for HF signal",
    )
    parser.add_argument(
        "--msg",
        metavar="TEXT",
        default="",
        help="Short message for HF signal (max 256 chars)",
    )
    ...
    if args.list:
        return cmd_list()
    if args.hf:
        return cmd_hf(args.hf, args.gps, args.msg)
